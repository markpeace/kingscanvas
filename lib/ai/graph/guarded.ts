...
export type RunResult = { output: string; mode: "langgraph" | "direct-fallback"; llmMode: ModelMode }

export async function runGuarded(input: string, mode?: ModelMode): Promise<RunResult> {
  // Try dynamic import of LangGraph so we don't crash on incompatible runtimes.
  const llmMode = resolveModelMode(mode)
  try {
    ...
    async function node(state: SimpleState): Promise<SimpleState> {
      const model = getChatModel({ mode: llmMode })
    ...
    return { output: String(result.output ?? ""), mode: "langgraph", llmMode }
  } catch {
    ...
    const content =
      typeof (res as any) === "string"
        ...
        : Array.isArray((res as any).content)
          ? (res as any).content.map((c: any) => (typeof c?.text === "string" ? c.text : "")).join("\n")
          : (res as any).content ?? ""
    return { output: String(content || ""), mode: "direct-fallback", llmMode }
  }
}
